...
def plot_df(df, color, xaxis, yaxis, ma=1, label=""):
    ...
    df[[xaxis, yaxis]].to_csv(f"{args.method}_raw_data.csv", index=False)
    ...
    textstr = f"Mean: {np.mean(mean):.2f}\nStd: ±{np.std(std):.2f}"
    plt.text(
        0.02, 0.95,
        textstr,
...
if __name__ == "__main__":
    ...
    prs.add_argument("--method", required=True, choices=["RL", "FIX"], help="Type of TL control\n")
    prs.add_argument("--time_ID", required=True, help="ID to save graph on specific folder\n")
    ...
    plt.savefig(f"outputs/{args.time_ID}/result_{args.method}.png")
